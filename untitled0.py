...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan  5 19:35:27 2018
...
def tw_search(api):
    ...
    csvWriter.writerow(["username","created", "text", "retwc", "hashtag", "followers", "friends","location","age"])
    ...
    for tweet in tweepy.Cursor(api.search,
        ...
        #authorage = tweet.author.age
        
        #TECHNOLOGY INFO
        geoenable = tweet.author.geo_enabled     #is author/user account geo enabled?
        source    = tweet.source                 #platform source for tweet
		# Dongho 03/28/16
        csvWriter.writerow([username,created, str(text).encode("utf-8"), retwc, hashtag, followers, friends,authorloc])
        counter = counter +1
        if (counter == c):
            break
